...
def _create_wrapper_executable(gbn_path: Path) -> Path:
    ...
    wrapper = f"#!/usr/bin/env bash\nexec python3 \"{parser_path}\" \"{gbn_path.resolve()}\" \"$@\"\n"
...
def _resolve_lib_path(path_text: str, current_dir: Path, core_dir: Path) -> Path | None:
    ...
    Search order mirrors engine.resolve_path() exactly:
      ...
      2. libs/ directory sibling of Core/
    """
    ...
    # Mirror engine: search libs/ directory (sibling of Core/)
    ...
    if "/" not in path_text and stem_only:
        if stem_only.lower() != "stdutils":
            libs_dir = (core_dir / ".." / "libs").resolve()
            for ext in SUPPORTED_EXTS:
...
def _collect_imported_gbn_libs(gbn_path: Path) -> list[Path]:
    ...
    # core_dir = directory containing compiler.py and engine.py (the Core/ package)
    ...
    while queue:
        ...
        for raw_line in text.splitlines():
            ...
            resolved = _resolve_lib_path(raw_path, current.parent, core_dir)
            if resolved is None:
