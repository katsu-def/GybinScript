...
`#reserved` se usa para declarar elementos de un script privados, es decir, no pueden usarse fuera del script donde se declaró:
...
> ! Si la clase padre ya tiene un método `init()`, la clase hija debe usar `__init__()` en su lugar para evitar conflicto de nombres. La clase hija hereda todos los campos y métodos de la clase padre.
...
var espada: Item = $Item("Sword", $ItemType.WEAPON)
```
...
Pausa la ejecución y devuelve un resumen de la memoria:
